...
public class BuildCustomProcess
{
    ...
    public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject)
    {
        ...
        int newversion = int.Parse(splits[splits.Length-1]) + 1;

        string newCompleteVersion = "";
        for (int i = 0; i < splits.Length - 1; i++) {
            newCompleteVersion += splits[i] + ".";
        }

        PlayerSettings.bundleVersion = newCompleteVersion + newversion.ToString();
