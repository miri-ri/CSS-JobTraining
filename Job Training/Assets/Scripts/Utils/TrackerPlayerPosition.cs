...
public class TrackerPlayerPosition : MonoBehaviour
{
    ...
    private void Start()
    {
        if (MagicRoomManager.instance != null)
        {
            ...
            {
                sensorDisallinment = new Vector3(MagicRoomManager.instance.systemConfiguration.floorOffsetX, MagicRoomManager.instance.systemConfiguration.floorOffsetY, MagicRoomManager.instance.systemConfiguration.floorOffsetZ);
                standardizedFloorSize = new Vector3(MagicRoomManager.instance.systemConfiguration.floorSizeX / Mathf.Pow(2.74f, 2), MagicRoomManager.instance.systemConfiguration.floorSizeY / Mathf.Pow(2.88f, 2), MagicRoomManager.instance.systemConfiguration.floorSizeZ / Mathf.Pow(2.88f, 2));
            }
        }
    }
    ...
    private void Update()
    {
        if (skelPosition != null)
        {
            foreach (var p in trackingSchema)
            {
                ...
                //4.5) apply absolute shift
                newposition += interactiveArea.getAbsoluteShift();
    ...
    private void ManageSkeleton(Dictionary<ulong, Skeleton> skel)
    {
        ...
        } else if (playerIdentifier == PlayerToFollow.LeftMost) {
            ulong closestId = skel.Keys.ToArray()[0];
            float distance = -Mathf.Infinity;
            foreach (ulong id in skel.Keys)
            {
                if (skel[id].SpineBase.x < distance && skel[id].SpineBase.z > 0)
                {
                    closestId = id;
                    distance = skel[id].SpineBase.x;
                }
            }
            skelPosition = skel[closestId];
        }
        else if (playerIdentifier == PlayerToFollow.RighMost)
        {
            ulong closestId = skel.Keys.ToArray()[0];
            float distance = Mathf.Infinity;
            foreach (ulong id in skel.Keys)
...
{
    Closest_player, Player1, Player2, Player3, Player4, Player5, Player6, RighMost, LeftMost
}
...
public class BoxArea
{
    ...
    public float absoluteshiftX = 0;
    public float absoluteshiftY = 1;
    public float absoluteshiftZ = 1;
    ...
    internal Vector3 getAbsoluteShift() {
        return new Vector3(absoluteshiftX, absoluteshiftY, absoluteshiftZ);
    }
}
